...
TaskManager.defineTask(BACKGROUND_SCAN_TASK, async ({ data, error }: { data: any; error: any }) => {
  if (error) {
...
const BLEBeaconScanner = () => {
  ...
  useEffect(() => {
    ...
    const bleManager = new BleManager({
      ...
      restoreStateFunction: (restoredState) => {
        log('BLE Manager state restored', { hasRestoredState: !!restoredState });
      },
    });
    ...
    const processDevices = (devices: any[]) => {
      ...
      setBeacons(prevBeacons => {
        ...
        devicesWithServiceData.forEach(device => {
          const existingIndex = updatedBeacons.findIndex(b => b.id === device.id);
      ...
      try {
        ...
        if (!locationEnabled) {
          ...
          if (!enabled) {
            ...
            Alert.alert('Location Required', 'Bluetooth scanning requires location services to be enabled.');
            return false;
          }
        }
        ...
        if (apiLevel >= 31) {
          ...
          const requiredPermissions = apiLevel >= 31 
            ...
            : [
                PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
                ...(apiLevel >= 29 ? [PermissionsAndroid.PERMISSIONS.ACCESS_BACKGROUND_LOCATION] : [])
              ];
  ...
  const startScan = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (status !== 'granted') {
        ...
        Alert.alert('Notifications Disabled', 'You will not receive alerts for nearby beacons.');
      }
      ...
      if (!permissionsGranted) {
        ...
        log(errorMsg);
        Alert.alert('Permission Required', 'Bluetooth and Location permissions are required to scan for beacons.');
        return false;
      }
      ...
      if (state !== 'PoweredOn') {
        ...
        log(errorMsg);
        Alert.alert('Bluetooth Off', 'Please enable Bluetooth to scan for beacons.');
        return false;
      }
      ...
      manager.startDeviceScan(
        ...
        (error, device) => {
          ...
          if (device?.serviceData) {
            ...
            setBeacons(prevBeacons => {
              const existingIndex = prevBeacons.findIndex(b => b.id === device.id);
              const deviceInfo: BeaconDevice = {
                id: device.id,
                name: device.name || 'Unknown',
  ...
  return (
    <View style={{ flex: 1, padding: 20 }}>
      <Text style={{ fontSize: 20, marginBottom: 20, fontWeight: 'bold' }}>BLE Beacon Scanner</Text>
      ...
      <View style={{ marginBottom: 10, padding: 10, backgroundColor: '#f5f5f5', borderRadius: 5 }}>
        <Text style={{ fontWeight: 'bold' }}>Status: {isScanning ? 'Scanning...' : 'Idle'}</Text>
        <Text>Beacons with service data: {beacons.length}</Text>
      </View>
      
      <Text style={{ marginTop: 10, fontSize: 16, fontWeight: 'bold' }}>Beacons with Service Data:</Text>
      {beacons.length === 0 ? (
        <Text style={{ marginTop: 10, fontStyle: 'italic', color: '#666' }}>
          No beacons with service data found yet. Start scanning to discover devices.
        </Text>
      ) : (
        [...beacons]
          .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
          .map((beacon, index) => (
            <View key={`${beacon.id}-${index}`} style={{
              marginTop: 10,
              padding: 10,
              backgroundColor: index % 2 === 0 ? '#f9f9f9' : '#fff',
              borderRadius: 5,
              borderLeftWidth: 3,
              borderLeftColor: beacon.rssi > -70 ? '#4CAF50' : beacon.rssi > -85 ? '#FFC107' : '#F44336'
            }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ fontWeight: 'bold' }}>{beacon.name || 'Unknown Device'}</Text>
                <Text style={{ color: '#666' }}>{new Date(beacon.timestamp).toLocaleTimeString()}</Text>
              </View>
              ...
              {beacon.serviceData && (
                <View style={{ marginTop: 8, padding: 8, backgroundColor: '#f0f0f0', borderRadius: 4 }}>
                  <Text style={{ fontWeight: '600', marginBottom: 4 }}>Service Data:</Text>
                  {Object.entries(beacon.serviceData).map(([key, value]) => (
                    <Text key={key} style={{ fontSize: 12, fontFamily: 'monospace' }}>
