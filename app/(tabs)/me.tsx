...
import { Image, Linking, Pressable, StyleSheet, Text, TouchableOpacity, View, ActivityIndicator } from 'react-native';
      ...
      <View style={{ flex: 1, width: '100%', alignItems: 'center' }}>
        <View style={styles.userContainer}>
          <View style={styles.userRow}>
            <View style={[styles.avatarContainer, { backgroundColor: profile?.color || '#4A89EE' }]}>
              <Text style={styles.avatarText}>
                {profile?.name ? profile.name.charAt(0).toUpperCase() : '?'}
              </Text>
            </View>
            <View style={styles.userInfo}>
              <Text style={[styles.nameText, { fontSize: 24 }]}>{profile?.name || 'Käyttäjä'}</Text>
              {profile?.class && <Text style={[styles.nameText, { fontSize: 16, color: '#666' }]}>{profile.class}</Text>}
            </View>
          </View>
        </View>
