...
export default function HomeScreen() {
  ...
  const fetchFriendLocations = useCallback(async () => {
    try {
      ...
      const friendsWithLocs: FriendWithLocation[] = friends.map(friend => {
        ...
        return {
          ...friend,
          location: friendLocation ? [friendLocation.x, friendLocation.y] : null, // Fixed: x=longitude, y=latitude
          locationData: friendLocation
        };
      });
      ...
      console.log('🧑‍🤝‍🧑 Friend locations updated:', {
        ...
        friendsWithValidLocations: friendsWithLocs.filter(f => f.location).length,
  ...
  const friendsGeoJSON = useMemo(() => {
    ...
    const friendsToShow = friendsWithLocations
      .filter(friend => {
    ...
    friendsWithLocations.forEach((friend, index) => {
      console.log(`  ${index + 1}. ${friend.name}:`, {
        ...
        willBeDisplayed: friend.location && friend.locationData?.floor === selectedFloor.toString()
      });
    });
    ...
    if (features.length > 0) {
      ...
      console.log('🌍 Coordinate analysis:', {
        ...
        isNearOtaniemi: Math.abs(lat - 60.184) < 0.01 && Math.abs(lng - 24.818) < 0.01,
        mapCenter: [24.818510511790645, 60.18394233125424],
        distanceFromCenter: Math.sqrt(Math.pow(lng - 24.818510511790645, 2) + Math.pow(lat - 60.18394233125424, 2))
      });
    }
