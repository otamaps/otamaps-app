...
export default function WilmaRoomScheduleScreen() {
  ...
  const load = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      setSchedule(await fetchWilmaRoomSchedule(id, formatFinnishDate(weekMonday), { forceRefresh }));
    } catch (cause) {
  ...
  return (
    <SafeAreaView style={[styles.container, isDark && styles.containerDark]} edges={["top"]}>
      ...
      <View style={[styles.header, isDark && styles.borderDark]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.headerTitle, isDark && styles.textLight]}>{schedule?.room.code ?? code ?? "Tila"}</Text>
          <Text style={styles.headerSubtitle}>{schedule?.room.name ?? name ?? ""}</Text>
        </View>
      </View>
              ...
              <View key={day} style={styles.daySection}>
                ...
                {lessons.length ? lessons.map((lesson, lessonIndex) => (
                  ...
                  <View style={[styles.emptyDay, isDark && styles.emptyDayDark]}>
                    <Text style={[styles.emptyDayText, isDark && styles.mutedDark]}>Ei varauksia</Text>
                  </View>
                )}
              </View>
            );
...
const styles = StyleSheet.create({
  ...
  weekNavDark: { backgroundColor: "#232427", borderBottomColor: "#333" },
  navButton: { padding: 8 },
  ...
  emptyDayDark: { borderColor: "#353535", backgroundColor: "#232427" },
