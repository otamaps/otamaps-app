...
import {
  fetchWilmaTeacherSchedule,
  WilmaTeacherSchedule,
...
export default function WilmaTeacherScheduleScreen() {
  ...
  const load = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        setSchedule(
          await fetchWilmaTeacherSchedule(id, formatFinnishDate(weekMonday), {
            forceRefresh,
          }),
        );
      } catch (cause) {
  ...
  const lessonsByDay = useMemo(
    () =>
      schedule?.lessons.reduce<Record<number, WilmaTeacherSchedule["lessons"]>>(
        (result, lesson) => {
  ...
  // over for 30+ minutes, or the teacher has nothing today at all — the next
  // school day, matching how the personal schedule reads. Only meaningful
        ...
        <View style={[styles.header, isDark && styles.headerDark]}>
          ...
          <View style={{ flex: 1 }}>
            <Text
              ...
              numberOfLines={1}
            >
              {name || schedule?.teacher.name || "Opettaja"}
            </Text>
            <Text style={styles.headerSubtitle} numberOfLines={1}>
              {schedule?.teacher.code || code || "Lukujärjestys"}
            </Text>
          </View>
        </View>
          ...
          <View style={styles.bodyWrap}>
            ...
            >
              {SCHOOL_DAYS.map((dayName, index) => {
                ...
                // (today's last lesson is long over), today itself is done too
                ...
                return (
                  ...
                  >
                    ...
                    {lessons.length ? (
                      // The day's lessons read as one continuous card with
                      ...
                      <View>
                        {lessons.map((lesson, lessonIndex) => {
                          ...
                          // day would make today's lessons darker than a
                      ...
                      >
                        <Text style={styles.emptyDayText}>Ei oppitunteja</Text>
                      </View>
                    )}
                  </View>
                );
