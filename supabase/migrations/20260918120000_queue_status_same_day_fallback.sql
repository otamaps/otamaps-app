-- Ruokalinjasto: bound the stale fallback in `get_queue_statuses` to the
-- current school day.
--
-- 20260824120000 stopped the panel collapsing to "no data" the moment the
-- current slot had nothing, by falling back to the most recent known reading
-- across every source. It put no age limit on that reading, so outside a busy
-- lunch the panel could surface a level observed days earlier and the app
-- would label it "Edellisen 15 minuutin jakson tieto" — a queue length from
-- last week presented as the slot that just passed.
--
-- The fallback now only considers readings observed on the same local day as
-- the current slot. Any earlier slot that day still counts, not just the one
-- immediately before: within the 10:45-12:30 window that is at most a
-- 7-slot reach, and the app always renders the reading with how old it is.
-- With nothing from today, `status_level` goes null as before and the app
-- says so outright.
--
-- This file is self-contained: it carries the whole of 20260824120000 as well,
-- so a database still on the 24-column build from 20260817002500 (no
-- `status_is_stale`, no fallback at all) comes fully up to date by running
-- this one migration.
--
-- The function is dropped and recreated rather than replaced, because adding
-- `status_is_stale` changes its return type and Postgres refuses a
-- `create or replace` across that. Dropping takes the grants with it, so they
-- are reinstated below.

begin;

-- ---------------------------------------------------------------------------
-- 1. Staff-verified reports expire after 15 minutes, matching the community
--    slot length. Carried over from 20260824120000; a no-op where it ran.
-- ---------------------------------------------------------------------------
...
as $$
  ...
  stale as (
    ...
    left join lateral (
      ...
      where candidate.level is not null
...
-- Reinstated after the drop above.
...
comment on function public.get_queue_statuses() is
  'Safe per-area queue aggregate for authenticated users. Falls back to the most recent reading from the same local day once the current slot has none, flagged by status_is_stale. Columns are append-only: never reorder or remove one, because already-installed app builds read them by name.';
