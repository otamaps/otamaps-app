...
Use [Supabase and Google auth](../../concepts/authentication/supabase-and-google-auth) for the broader session authority model. Use [Wilma endpoints and SecureStore keys](../../reference/wilma/endpoints-and-securestore-keys) for exact endpoint names and stored-key lookup, [client caches](../../reference/storage/client-caches) for local persistence boundaries, [runtime and build config](../../reference/configuration/runtime-and-build-config) for public API and Wilma flag configuration, and [server deployment](../../guides/deployment/server-deployment) when the broker's OtaMaps API routes or production hostnames change.
