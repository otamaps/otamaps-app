...
The room-label path treats display fields as nullable even though the shared room type still marks them as strings. `roomsGeoJSON` converts nullish `room_number` and `title` values to trimmed strings before building feature properties, and `getRoomNumberMaxTextSize` accepts an unknown room number before counting characters [@map-screen]. This guard is intentional: a runtime map crash showed `Array.from(roomNumber)` receiving `null` from live room data during label sizing [@map-ui-session].
