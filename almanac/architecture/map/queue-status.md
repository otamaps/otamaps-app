...
Future queue work should preserve that distinction. The admin screen can show the configured-window label as anonymous location samples, not people, because `get_admin_queue_activity()` returns a sample count rather than a distinct-user estimate and the visible window length comes from the queue status row [@admin-screen] [@queue-service] [@queue-migration]. Showing occupancy claims, per-person counts, or raw sample history would be a product and privacy change, not a small UI copy change.

## Related Pages

Use [map, social, location, queue, and consent tables](../../reference/supabase/map-social-and-location-tables) for exact queue table and RPC names. Use [onboarding and consent preferences](../auth/onboarding-and-consent-preferences) for the anonymous analytics preference that controls crowd-sample writes. Use [server deployment](../../guides/deployment/server-deployment) when a queue change depends on Supabase migrations, PostgREST schema reload, or production RPC grants. Use [test coverage](../../reference/testing/test-coverage) for the package-wired queue formatting test scope. Use [lunch-shift extraction via LLM paste, not a parser](../../decisions/mobile/lunch-shift-llm-extraction) for the same admin stack's other periodic-data feature: why lunch-shift PDFs are extracted by pasting into Claude rather than parsed in code.
