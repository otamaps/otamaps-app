...
The August 12, 2026 version `0.4.2` update produced group `ba509490-0d5b-4303-800d-67dfe21911b6` with message `0.4.2 - schedule sharing, canteen reporting and menu`, runtime `0.4.1`, Android update id `019ff80f-99b0-7570-b8be-9f3a37124d7a`, and iOS update id `019ff80f-99b0-7cad-8b1d-9d36cf45d011` [@required-update-session]. That session verified the published group with `eas update:view <group> --json` without `--non-interactive`, because this project-local EAS CLI rejected `--non-interactive` for `update:view` [@required-update-session].

`npx eas update:view <group> --json --non-interactive` failed in the August 12 session because this EAS CLI command rejected `--non-interactive`; use `channel:view`, `update:list`, direct manifest probes, or rerun `update:view` without that flag when an exact group view is needed [@required-update-session].
...
Do not call an OTA release fully proven from manifest checks alone. The Wilma timeout update was published and remotely visible, but a real successful Wilma login still needed an on-device test; if login failed again, the new telemetry was expected to distinguish a 45-second auth timeout from a server response error [@update-session] [@wilma-auth-broker] [@sentry-runtime]. The August 12, 2026 update was remotely visible on the production branch for iOS and Android, the matching Supabase migration was applied, and reciprocal schedule-sharing visibility was verified in production, but iOS simulator testing was skipped and an actual canteen contribution could not be submitted because verification happened outside the 10:45-12:30 reporting window [@required-update-session].
